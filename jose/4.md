...
- 자바스크립트가 서서히 다양한 작업을 수행하게 되면서 자바스크립트를 모듈화하는 방안으로 **CommonJS**와 AMD(Asynchronous Module Definition)가 나왔다.
...
`renderToString` 과 결과물이 완전히 동일하지만 브라우저에서는 사용하는 것이 불가능하다. `renderToNodeStream`의 결과물은 Node.js의 ReadableStream(utf-8 인코딩된 바이트 스트림)이여서 Node.js에서만 사용할 수 있다.
...
3000번 포트를 이용하는 HTTP 서버를 만드는 코드. Node.js의 기본 라이브러리.
...
여기서는 indexFront, indexEnd 로 index.html이 반반 나뉘어진 코드에서 각각 응답을 기록하고 청크단위로 생성하여 나머지 반을 붙여서 최종 결과물을 브라우저에 리턴한다. 결과물은 동일하고 서버에서의 차이점말곤 없다.
...
_document.tsx는 애플리케이션의 HTML을 초기화함.
...
HTML이 getServerSideProps 기반으로 렌더링되어 있고 여기서 눈여겨봐야 할 것은 `id="__NEXT_DATA__"` 의 script 이다.
